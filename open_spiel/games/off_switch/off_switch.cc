...
namespace open_spiel
{
  namespace off_switch
  {
    ...
    class OffSwitchObserver : public Observer
    {
    public:
      OffSwitchObserver(IIGObservationType iig_obs_type)
      ...
      {
        const OffSwitchState &state =
            open_spiel::down_cast<const OffSwitchState &>(observed_state);
    ...
    OffSwitchState::OffSwitchState(std::shared_ptr<const Game> game)
        : State(game),
    ...
    int OffSwitchState::CurrentPlayer() const
    {
      if (IsTerminal())
      {
    ...
    std::string OffSwitchState::ActionToString(Player player, Action move) const
    {
    ...
    bool OffSwitchState::IsTerminal() const { return winner_ != kInvalidPlayer; }
    ...
    {
      const OffSwitchGame &game = open_spiel::down_cast<const OffSwitchGame &>(*game_);
    ...
    void OffSwitchState::InformationStateTensor(Player player,
    ...
    std::vector<std::pair<Action, double>> OffSwitchState::ChanceOutcomes() const
    {
    ...
    bool OffSwitchState::DidBet(Player player) const
    {
      if (first_bettor_ == kInvalidPlayer)
      {
        return false;
    ...
    std::unique_ptr<State> OffSwitchState::ResampleFromInfostate(
        ...
        : Game(kGameType, params), num_players_(ParameterValue<int>("players"))
    {
      ...
      default_observer_ = std::make_shared<OffSwitchObserver>(kDefaultObsType);
      info_state_observer_ = std::make_shared<OffSwitchObserver>(kInfoStateObsType);
      private_observer_ = std::make_shared<OffSwitchObserver>(
          IIGObservationType{/*public_info*/ false,
                             /*perfect_recall*/ false,
                             /*private_info*/ PrivateInfoType::kSinglePlayer});
      public_observer_ = std::make_shared<OffSwitchObserver>(
          IIGObservationType{/*public_info*/ true,
                             /*perfect_recall*/ false,
                             /*private_info*/ PrivateInfoType::kNone});
    }

    std::unique_ptr<State> OffSwitchGame::NewInitialState() const
    {
      return std::unique_ptr<State>(new OffSwitchState(shared_from_this()));
    }

    std::vector<int> OffSwitchGame::InformationStateTensorShape() const
    {
    ...
    std::vector<int> OffSwitchGame::ObservationTensorShape() const
    {
      // One-hot for whose turn it is.
      // One-hot encoding for the single private card. (n+1 cards = n+1 bits)
    ...
    double OffSwitchGame::MaxUtility() const
    {
    ...
    double OffSwitchGame::MinUtility() const
    {
      // In poker, the utility is defined as the money a player has at the end
      // of the game minus then money the player had before starting the game.
    ...
    std::shared_ptr<Observer> OffSwitchGame::MakeObserver(
        absl::optional<IIGObservationType> iig_obs_type,
        const GameParameters &params) const
    {
      if (params.empty())
      {
        return std::make_shared<OffSwitchObserver>(
    ...
    {
      SPIEL_CHECK_TRUE(
          dynamic_cast<OffSwitchGame *>(const_cast<Game *>(&game)) != nullptr);
